...
export default class AUser {
  ...
  public static register(
    ...
    SLog.log(LogType.Info, "register", "check url, check params", {
      url,
      data,
    });
  ...
  public static minusUserPoints(
    ...
    console.log("Sending request to subtract points:", {
      user_id,
      point,
      report_id,
    });
    ...
    axios
      .post(`${ReactAppUrl.API_BASE_URL}/reports/minusUserPoints`, {
        user_id,
        point,
        report_id,
      })
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          onNext({
            success: false,
            message: response.data.message || "Failed to subtract points.",
          });
        }
      })
      .catch((error) => {
        ...
        onNext({
          success: false,
          message: "Failed to subtract points due to server error.",
        });
      })
      .finally(() => {
        // Kết thúc loading
  ...
  public static lockUserAccount(
    ...
    onLoading(true);
    console.log(
      `Gửi request để khóa tài khoản cho user_id: ${user_id} và report_id: ${report_id}`
    );
    console.log(`${ReactAppUrl.API_BASE_URL}/reports/lockUserAccount`);

    // Gửi request POST đến BE với user_id, report_id và permissionIds
    axios
      .post(`${ReactAppUrl.API_BASE_URL}/reports/lockUserAccount`, {
        user_id,
