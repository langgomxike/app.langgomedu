...
export default function MessageItem({
    ...
    //states
    let content;
    const [ofMine, setOfMine] = useState(false);

    // main content
    if (message.file && message.file.id >= 1) {
        if (message.is_image) {
            content = (
                <Image
    ...
    //mine  1, from user 0
    //mine 0, to user 0
    if ((ofMine && !message.from_user_status) || (!ofMine && !message.to_user_status)) {
        content = (
            <Text
                style={[
                    styles.content,
    ...
    //effects
    useEffect(() => {
        setOfMine(accountContext.account?.id === message.from_user?.id)
